...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    quitBtnEl.addEventListener('click', () => window.location.href = 'dashboard.html');
    ...
    function slideCanvas(){ if (score>slideStartScore){ if (!slideInit){ slideInit=true; boundsX=(window.innerWidth-wrapper.clientWidth)/2; boundsY=(window.innerHeight-wrapper.clientHeight)/2; const s=2+Math.random()*3; slideVX=s*(Math.random()<0.5?1:-1); slideVY=s*(Math.random()<0.5?1:-1); } slideX+=slideVX; slideY+=slideVY; if (slideX> boundsX){ slideX=boundsX; slideVX*=-1; } if (slideX< -boundsX){ slideX=-boundsX; slideVX*=-1; } if (slideY> boundsY){ slideY=boundsY; slideVY*=-1; } if (slideY< -boundsY){ slideY=-boundsY; slideVY*=-1; } wrapper.style.transform = `translate(${slideX}px,${slideY}px)`; }}
